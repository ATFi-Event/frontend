...
function CustomDropdown({ isOpen, onClose, user, logout }: CustomDropdownProps) {
            ...
            {profileLoading ? (
              ...
              <>
                <h3 className="font-semibold text-white truncate">
                  {profileData?.name || user?.farcaster?.displayName || user?.twitter?.username || 'User'}
                </h3>
                <p className="text-sm text-gray-400 truncate">
                  {profileData?.email || user?.email || 'No email'}
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  Balance: {profileData?.balance || user?.wallet?.balance || '0.00'} USDC
                </p>
  ...
  return (
    ...
    >
      ...
      <Link href="/home" className="flex items-center">
        <h1 className="text-[#939094] font-bold text-xl hover:text-white transition-colors">ATFI</h1>
      </Link>
