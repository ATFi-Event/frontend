...
export default function DepositModal({
  ...
  const handleDeposit = async () => {
    ...
    console.log(
      `Using wallet for deposit: ${preferredWallet.name} (${preferredWallet.address})`
    );
    console.log(
      `Wallet type: ${
    ...
    try {
      ...
      if (balanceNum < requiredBalance) {
        throw new Error(
          `Insufficient USDC balance. Required: ${requiredBalance.toFixed(
            2
          )} USDC, Available: ${balanceNum.toFixed(2)} USDC`
        );
      }
      ...
      const approveCalldata = encodeApproveUSDC(
        eventData.vault_address,
        eventData.stake_amount
      );
  ...
  const checkBalance = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `💰 Balance check from profile API for ${preferredWallet.address}: ${profileData.balance} USDC`
        );
      } else {
  ...
  const checkBalanceFromBlockchain = async (preferredWallet: WalletInfo) => {
    try {
      ...
      console.log(
        `💰 Blockchain balance check for ${preferredWallet.address}: ${balance} USDC`
      );
    } catch (error) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-medium text-white">
                  {WalletService.getPreferredWallet(user)?.type === "gmail"
                    ? "📧 Gmail Wallet"
                    : "🦊 MetaMask"}
                </p>
                <p className="text-xs text-gray-400">
                  {WalletService.getPreferredWallet(user)?.address?.slice(0, 6)}
                  ...
                  {WalletService.getPreferredWallet(user)?.address?.slice(-4)}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white/5 rounded-xl p-4 mb-6 border border-white/10">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-white font-bold text-lg">
              {requiredBalance.toFixed(2)} USDC
            </span>
          </div>
          <div className="flex justify-between items-center mb-3">
            ...
            {!hasEnoughBalance && (
              <p className="text-red-300 text-sm mt-2">
                ...
                <span className="font-bold">
                  {(requiredBalance - currentBalance).toFixed(2)} USDC
                </span>{" "}
                to participate
              </p>
            )}
          ...
          <div className="mb-6">
            ...
            <div className="w-full bg-white/20 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${
                    (depositState.step / depositState.totalSteps) * 100
                  }%`,
                }}
              ></div>
            </div>
          ...
          <div className="mb-6 bg-green-500/20 backdrop-blur-lg border border-green-500/50 text-green-100 px-6 py-6 rounded-lg">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-200 space-y-2">
                <p>
                  Your Event Check-in QR Code will be available during the event
                </p>
                <p className="text-xs text-gray-300">
                  You can access it from the event page when the event is live
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="bg-white p-4 rounded-xl">
                ...
                <div className="text-center text-xs text-gray-600">
                  ...
                  <p>
                    Wallet:{" "}
                    {user &&
                      WalletService.getPreferredWallet(user)?.address?.slice(
                        0,
                        6
                      )}
                    ...
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400">
                Approve USDC for the vault contract
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-full bg-gray-600 flex items-center justify-center text-xs text-white">
              ...
              <span className="text-xs text-gray-400">
                Deposit USDC to the event vault
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 rounded-full bg-gray-600 flex items-center justify-center text-xs text-white">
