...
export default function OverviewContent() {
  ...
  const executeLiveNow = async () => {
    ...
    try {
      ...
      console.log(
        `Using wallet for Go Live: ${preferredWallet.name} (${preferredWallet.address})`
      );
      console.log(
        `Wallet type: ${
      ...
      const { VAULT_FUNCTION_SIGNATURES } = await import(
        "@/utils/contracts/vault"
      );
      ...
      const updateEventStatus = async () => {
        ...
        console.log("📡 Attempting to update event status:", {
          eventId,
          updatePayload,
        });
        ...
        const endpoints = [
          {
            url: `${process.env.NEXT_PUBLIC_API_URL}/api/v1/events/${eventId}/status`,
            method: "PATCH",
          },
          {
            url: `${process.env.NEXT_PUBLIC_API_URL}/api/v1/events/${eventId}`,
            method: "PATCH",
          },
          {
            url: `${process.env.NEXT_PUBLIC_API_URL}/api/v1/events/${eventId}/status`,
        ...
        for (const endpoint of endpoints) {
          try {
            console.log(
              `🔄 Trying endpoint: ${endpoint.method} ${endpoint.url}`
            );
            ...
            if (response.ok) {
              ...
              console.log(
                "✅ Event status updated successfully in backend:",
                responseData
              );
              ...
              console.error(
                `❌ Backend API error for ${endpoint.method} ${endpoint.url}:`,
                response.status,
                errorText
              );
              ...
              if (endpoint === endpoints[endpoints.length - 1]) {
                throw new Error(
                  `All endpoints failed. Last error: ${response.status} - ${errorText}`
                );
              }
            }
          } catch (endpointError) {
            console.error(
              `❌ Failed to connect to endpoint ${endpoint.url}:`,
              endpointError
            );
      ...
      try {
        ...
        setError(
          `⚠️ Transaction successful but backend update failed. The event may take a moment to reflect changes. Please refresh the page. Error: ${
  ...
  const executeSettleEvent = async () => {
    ...
    try {
      ...
      const participantsResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/events/${eventData.event.event_id}/participants`
      );
      ...
      if (attendedParticipants.length === 0) {
        throw new Error(
          "No participants have attended this event yet. Cannot settle."
        );
      }
      ...
      const arrayLength = attendedParticipants.length
        .toString(16)
        .padStart(64, "0");
      data += arrayLength;
      ...
      if (confirmed) {
        ...
        try {
          ...
          console.error(
            "❌ Failed to update backend settlement status:",
            backendError
          );
        ...
        console.warn(
          "Transaction confirmation timed out, but the event may still be settled"
        );
  ...
  const getModalContent = () => {
    if (pendingAction === "liveNow") {
      return {
        title: "Go Live",
        description:
          "This will deposit funds to yield source and set your event status to LIVE. This action requires a blockchain transaction and gas fees.",
      };
    } else if (pendingAction === "settleEvent") {
      return {
        title: "Settle Event",
        description:
          "This will distribute rewards to attended participants and finalize your event. This action requires a blockchain transaction and gas fees.",
      };
    }
    return {
      title: "Confirm Transaction",
      description:
        "This action requires a blockchain transaction and gas fees.",
    };
  };

  const isOrganizer =
    user?.wallet?.address?.toLowerCase() ===
    eventData?.event.organizer_address?.toLowerCase();
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/5 backdrop-blur-lg rounded-3xl p-6 border border-white/10">
              ...
              <div className="text-center">
                <span
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold backdrop-blur-xl border ${
                    event.status === "LIVE"
            ...
            <div className="space-y-4">
              <div className="border-2 border-dashed border-white/20 rounded-xl p-8 text-center">
                ...
                <p className="text-gray-400 text-sm">
                  This feature will allow you to scan participant QR codes for
                  check-in
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">
                  Upload QR code image or use camera scan
                </p>
              </div>
            </div>
